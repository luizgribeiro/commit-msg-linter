...
const { isHuskyInstall } = require("./strategies/ruskyStrategy");
...
install = () => {
  if (isHuskyInstall()) {
    console.log("Using husky strategy");
  } else {
    ...
    if (
      exists(commitMsgHookFile) &&
      !fs.lstatSync(commitMsgHookFile).isSymbolicLink()
    ) {
      console.log(`${PACKAGE_NAME_LABEL}:`);
      console.log(
        `${PACKAGE_NAME_LABEL}: An existing git ${COMMIT_MSG_LABEL} hook detected`
      );
      ...
      if (exists(backup) || isFileWeCreated(commitMsgHookFile)) {
        ...
        console.log(
          `${PACKAGE_NAME_LABEL}: Old ${COMMIT_MSG_LABEL} hook backed up to ${old}`
        );
        console.log(`${PACKAGE_NAME_LABEL}:`);
      }
    }
    ...
    try {
      fs.writeFileSync(commitMsgHookFile, rules);
    } catch (e) {
      console.error(
        `${PACKAGE_NAME_LABEL}: ${chalk.red(
          "Failed to create the hook file in your .git/hooks folder because:"
        )}`
      );
      console.error(`${PACKAGE_NAME_LABEL}: ${chalk.red(e.message)}`);
      console.error(
        `${PACKAGE_NAME_LABEL}: ${chalk.red("The hook was not installed.")}`
      );
    }

    try {
      fs.chmodSync(commitMsgHookFile, "777");
    } catch (e) {
      const alert = chalk.red(
        `chmod 0777 the ${COMMIT_MSG_LABEL} file in your .git/hooks folder because:`
      );
